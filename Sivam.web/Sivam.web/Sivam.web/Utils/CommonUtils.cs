...
namespace Sivam.web.Utils
{
    public static class CommonUtils
    {
        public static string GenerateCode()
        {
            ...
            string rnum = _random.Next(1000, 9999).ToString("D4");
            return string.Concat("Shivam" + rnum);
        }

        public static string UploadImage(string base64, string fileName)
        {
            if (string.IsNullOrEmpty(base64))
            {
                return string.Concat(HttpContext.Current.Request.Url.GetLeftPart(UriPartial.Authority), "/images/placeholder.png");
            }
            else if (base64.Contains(".jpeg") || base64.Contains(".png")) //If we have already URL
            {
                return base64;
            }
            byte[] bytes = Convert.FromBase64String(base64);
            fileName = "/Banners/" + fileName + ".jpeg";
            string fullOutputPath = HttpContext.Current.Server.MapPath("~" + fileName);
            ...
            return string.Concat(HttpContext.Current.Request.Url.GetLeftPart(UriPartial.Authority), fileName);
        }
